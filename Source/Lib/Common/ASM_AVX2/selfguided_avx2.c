...
static INLINE void add_32bit_16x8(const __m512i neighbor, __m512i r[8]) {
    r[0] = _mm512_add_epi32(neighbor, r[0]);
    r[1] = _mm512_add_epi32(r[0], r[1]);
    r[2] = _mm512_add_epi32(r[1], r[2]);
    r[3] = _mm512_add_epi32(r[2], r[3]);
...
static __inline void transpose_32bit_8x16_avx2(const __m256i *const in, __m512i *const out) {
...
static AOM_FORCE_INLINE void integral_images_highbd_512(const uint16_t *src, int32_t src_stride,
    ...
    int y, i, j = 0;
    do {
        __m256i c_leftp = _mm256_setzero_si256();
        __m256i d_leftp = _mm256_setzero_si256();
        __m256i c_left = _mm256_setzero_si256();
        __m256i d_left = _mm256_setzero_si256();
        ...
        do {
            if((width - x ) > 15){
                __m256i s[8];
                __m256i r32[16], a32[16];
                __m512i t32[8], t32p[8], u32[8];
                ...
                __m512i resp[8], respb[8];
                    ...
                    transpose_16bit_16x16_avx2(s, r32);
                    for(i=0;i<16;i++){
                    ...
                    for(i=0;i<2;i++){
                        ...
                        //print_mat_256v(r32p);
                        a32p[0] = _mm256_madd_epi16(r32p[0], r32p[0]);
                        a32p[1] = _mm256_madd_epi16(r32p[1], r32p[1]);
                        a32p[2] = _mm256_madd_epi16(r32p[2], r32p[2]);
                        a32p[3] = _mm256_madd_epi16(r32p[3], r32p[3]);
                        a32p[4] = _mm256_madd_epi16(r32p[4], r32p[4]);
                        a32p[5] = _mm256_madd_epi16(r32p[5], r32p[5]);
                        a32p[6] = _mm256_madd_epi16(r32p[6], r32p[6]);
                        a32p[7] = _mm256_madd_epi16(r32p[7], r32p[7]);
                        //print_mat_256v(a32p);
                        add_32bit_8x8(c_leftp, a32p);
                        //print_mat_256v(a32p);
                        c_leftp = a32p[7];
                        transpose_32bit_8x8_avx2(a32p, a32p);
                        //print_mat_256v(a32p);
...
static INLINE void partial_transpose_8bit_16x16_512(const __m128i *const in, __m256i *const out) {
    ...
    out[4] = _mm256_setr_m128i( _mm_unpacklo_epi32(b4, b5),  _mm_unpacklo_epi32(b6, b7));
    out[5] = _mm256_setr_m128i( _mm_unpackhi_epi32(b4, b5),  _mm_unpackhi_epi32(b6, b7));
    out[6] = _mm256_setr_m128i( _mm_unpacklo_epi32(b12, b13),  _mm_unpacklo_epi32(b14, b15));
    out[7] = _mm256_setr_m128i( _mm_unpackhi_epi32(b12, b13),  _mm_unpackhi_epi32(b14, b15));
}

static __inline void cvt_16to32bit_8x16(const __m128i *const s, __m256i * r) {
    r[0] = _mm256_cvtepu16_epi32(s[0]);
    r[1] = _mm256_cvtepu16_epi32(s[1]);
    r[2] = _mm256_cvtepu16_epi32(s[2]);
    r[3] = _mm256_cvtepu16_epi32(s[3]);
    ...
    do {
        ...
        do {
            if((width - x) > 15){
                ...
                const __m512i d_top = _mm512_loadu_si512((__m512i *)(dt - buf_stride + x));
                //add_32bit_16x16(d_top, u32);// PV
                add_32bit_16x8(d_top, u32);// PV
                store_32bit_16x8(u32, dt + x, buf_stride);
...
void eb_apply_selfguided_restoration_avx2(const uint8_t *dat8, int32_t width, int32_t height,
    ...
    const __m512i xq0_512      = _mm512_set1_epi32(xq[0]);
    const __m512i xq1_512      = _mm512_set1_epi32(xq[1]);
    const __m256i rounding = round_for_shift(SGRPROJ_PRJ_BITS + SGRPROJ_RST_BITS);
    const __m512i rounding_512 = round_for_shift_512(SGRPROJ_PRJ_BITS + SGRPROJ_RST_BITS);
    ...
    if(avx512){
        if (!highbd) {
            ...
            do {
                ...
                do {
                    ...
                    if (params->r[0] > 0) {
                        ...
                        v_0                = _mm512_add_epi32(v_0, _mm512_mullo_epi32(xq0_512, f1_0));
                        v_1                = _mm512_add_epi32(v_1, _mm512_mullo_epi32(xq0_512, f1_1));
                    }
                    
                    if (params->r[1] > 0) {
                        const __m512i f2_0 = _mm512_sub_epi32(yy_loadu_512(&flt1[j + 0]), u_0);
                        const __m512i f2_1 = _mm512_sub_epi32(yy_loadu_512(&flt1[j + 16]), u_1);
                        v_0                = _mm512_add_epi32(v_0, _mm512_mullo_epi32(xq1_512, f2_0));
                        v_1                = _mm512_add_epi32(v_1, _mm512_mullo_epi32(xq1_512, f2_1));
                    }

                    const __m512i w_0 = _mm512_srai_epi32(_mm512_add_epi32(v_0, rounding_512),
                                                        SGRPROJ_PRJ_BITS + SGRPROJ_RST_BITS);
                    const __m512i w_1 = _mm512_srai_epi32(_mm512_add_epi32(v_1, rounding_512),
                                                        SGRPROJ_PRJ_BITS + SGRPROJ_RST_BITS);
                    ...
                    // 0, 1, 4, 5, 2, 3, 6, 7
                    const __m512i tmp = _mm512_packus_epi32(w_0, w_1);//printf("tmp  = %llx %llx %llx %llx %llx %llx %llx %llx\n",tmp[7],tmp[6],tmp[5],tmp[4],tmp[3],tmp[2],tmp[1],tmp[0]);
                    // 0, 1, 4, 5, 2, 3, 6, 7, 0, 1, 4, 5, 2, 3, 6, 7
                    const __m512i tmp2 = _mm512_packus_epi16(tmp, tmp);//printf("tmp2 = %llx %llx %llx %llx\n",tmp2[3],tmp2[2],tmp2[1],tmp2[0]);
                    // 0, 1, 2, 3, 4, 5, 6, 7, ...
                    const __m512i tmp3 = _mm512_permutexvar_epi32(idx, tmp2);//printf("tmp3 = %llx %llx %llx %llx %llx %llx %llx %llx\n",tmp3[7],tmp3[6],tmp3[5],tmp3[4],tmp3[3],tmp3[2],tmp3[1],tmp3[0]);
                    const __m256i res  = _mm512_castsi512_si256(tmp3);//printf("res  = %llx %llx %llx %llx\n",res[3],res[2],res[1],res[0]);
            ...
            do {
                ...
                do {
                    ...
                    __m512i       v_0   = _mm512_slli_epi32(u_0, SGRPROJ_PRJ_BITS);
                    __m512i       v_1   = _mm512_slli_epi32(u_1, SGRPROJ_PRJ_BITS);
                    //printf("u_0  = %llx %llx %llx %llx %llx %llx %llx %llx\n",u_0[7],u_0[6],u_0[5],u_0[4],u_0[3],u_0[2],u_0[1],u_0[0]);
                    //printf("u_1  = %llx %llx %llx %llx %llx %llx %llx %llx\n",u_1[7],u_1[6],u_1[5],u_1[4],u_1[3],u_1[2],u_1[1],u_1[0]);
                    ...
                    if (params->r[0] > 0) {
                        ...
                        v_0                = _mm512_add_epi32(v_0, _mm512_mullo_epi32(xq0_512, f1_0));
                        v_1                = _mm512_add_epi32(v_1, _mm512_mullo_epi32(xq0_512, f1_1));
                        //printf("f1_0  = %llx %llx %llx %llx %llx %llx %llx %llx\n",f1_0[7],f1_0[6],f1_0[5],f1_0[4],f1_0[3],f1_0[2],f1_0[1],f1_0[0]);
                        //printf("f1_1  = %llx %llx %llx %llx %llx %llx %llx %llx\n",f1_1[7],f1_1[6],f1_1[5],f1_1[4],f1_1[3],f1_1[2],f1_1[1],f1_1[0]);
                        //printf("v_1  = %llx %llx %llx %llx %llx %llx %llx %llx\n",v_1[7],v_1[6],v_1[5],v_1[4],v_1[3],v_1[2],v_1[1],v_1[0]);
                    }
                    ...
                    //printf("tmp  = %llx %llx %llx %llx %llx %llx %llx %llx\n",tmp[7],tmp[6],tmp[5],tmp[4],tmp[3],tmp[2],tmp[1],tmp[0]);
                    const __m512i tmp2 = _mm512_permutexvar_epi64(_mm512_set_epi64(7, 5, 3, 1, 6, 4, 2, 0), tmp);
                    //printf("tmp2 = %llx %llx %llx %llx %llx %llx %llx %llx\n",tmp2[7],tmp2[6],tmp2[5],tmp2[4],tmp2[3],tmp2[2],tmp2[1],tmp2[0]);
                    const __m512i res  = _mm512_min_epi16(tmp2, max);//printf("res  = %llx %llx %llx %llx\n",res[3],res[2],res[1],res[0]);
                    
                    yy_storeu_512(dst16 + j, res);
                    j += 32;
                } while (j < width);//while(1);
        ...
        do {
            ...
            do {
                ...
                const __m128i res  = _mm256_castsi256_si128(tmp3);//printf("res  = %llx %llx\n",res[1],res[0]);
        ...
        do {
            ...
            do {
                ...
                const __m256i res  = _mm256_min_epi16(tmp2, max);//printf("res  = %llx %llx %llx %llx\n",res[3],res[2],res[1],res[0]);
